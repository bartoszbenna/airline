...
export class BasketService {
  ...
  public async uploadBasket() {
    const basketUploadPromise = new Promise<IServerBasket>((resolve, reject) => {
      if (this.cookieService.check('token')) {
        const token = this.cookieService.get('token');
        const headers = new HttpHeaders({'x-access-token': token})
        this.http.post(this.basketApiUrl + 'upload', this.basketContent, {headers: headers}).subscribe((message: IServerBasket) => {
          this.basket = message;
          this.basketContent = [];

...
export class AccountGuardService implements CanActivate {
    ...
    async canActivate() {
        const routeActivationPromise = new Promise<boolean>((resolve, reject) => {
            if (this.loginService.currentStatus == null) {
                const loginStatusSubscription = this.loginService.getLoginStatusObservable().subscribe(message => {
                    if (message != null) {
                        resolve(message);
            ...
            else if (typeof this.loginService.currentStatus == 'boolean') {

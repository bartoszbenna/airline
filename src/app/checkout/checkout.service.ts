...
export class CheckoutService {
	...
	public async getOccupiedSeats(flightId: string) {
		const seatPromise = new Promise<string[]>((resolve, reject) => {
			this.http
				.get(this.searchApi + 'getOccupiedSeats', {
					params: { id: flightId },
				})
				.subscribe(
					(message: string[]) => {
	...
	public async makeReservation() {
		...
		if (this.cookieService.check('token')) {
			...
			const reservationPromise = new Promise<IServerReservation>(
				(resolve, reject) => {
					this.http
						.post(this.reservationApi + 'create', payload, {
							headers: headers,
						})
						.subscribe(
							(res: IServerReservation) => {
	...
	public async confirmReservation(reservationNumber: string) {
		const reservationConfirmationPromise = new Promise<boolean>(
			async (resolve, reject) => {
				const payload = {
					reservationNumber: reservationNumber,
				};
				this.http
					.patch(this.reservationApi + 'confirm', payload)
					.subscribe(
						(res: any) => {
							resolve(true);
						},

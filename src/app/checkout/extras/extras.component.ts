...
@Component({
	...
	animations: [
		trigger('SeatsInOutAnimation', [
			transition(':enter', [
				style({ height: 0 }),
				animate('0.5s ease-out', style({ height: '100vh' })),
			]),
			transition(':leave', [
				style({ height: '100vh' }),
...
export class ExtrasComponent implements OnInit {
	...
	async ngOnInit() {
		...
		if (this.basket.flights.length == 2) {
			this.oneWay = false;
			this.inboundOccupiedSeats =
				await this.checkoutService.getOccupiedSeats(
	...
	async next() {
		...
		if (!this.verifySeats()) {
			this.errorMessages.push(
				"Please make sure all seats are selected, or 'random' option is chosen."
			);
		}
		if (!this.verifyBaggage()) {
			this.errorMessages.push(
				'There has been a problem with the number of baggage.'
			);
		}
		this.showErrors = true;
		if (this.errorMessages.length == 0) {
			this.checkoutService.enterExtras(
				this.checkedBaggage,
				this.handBaggage,
				this.outboundSelectedSeats,
				this.inboundSelectedSeats
			);
			try {

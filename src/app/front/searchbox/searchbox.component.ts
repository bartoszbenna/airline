...
const DepArrDateValidator: ValidatorFn = (fg: FormGroup) => {
	...
	if (
		errors.depDateError == false &&
		errors.dateRangeError == false &&
		errors.returnDateNull == false
	) {
		return null;
	}
...
export class SearchboxComponent implements OnInit {
	...
	public searchForm = new FormGroup(
		{
			...
			outDate: new FormControl(new Date(2021, 4, 11, 2), [
				Validators.required,
			]),
	...
	ngOnInit(): void {
		...
		this.searchService.searchInsertionSubject
			.asObservable()
			.subscribe((message) => {

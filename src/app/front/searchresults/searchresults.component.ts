...
export class SearchresultsComponent implements OnInit {
	...
	ngOnInit(): void {
		...
		this.searchService.resetResultsSubject
			.asObservable()
			.subscribe((message) => {
				this.resetResults();
			});

		this.searchService.isWaitingForResults
			.asObservable()
			.subscribe((message) => {
		...
		this.searchService.searchResults.asObservable().subscribe((message) => {
			...
			this.outboundFlights.push({
				date: new Date(earliestOutbound),
				flights: [],
			});
			this.outboundFlights.push({
				date: new Date(this.searchForm.outDate),
				flights: [],
			});
			this.outboundFlights.push({
				date: new Date(latestOutbound),
				flights: [],
			});
			...
			for (let flight of this.searchResults.outbound) {
				for (let day of this.outboundFlights) {
					if (
						moment(flight.depDate).isSame(day.date, 'days') &&
						moment(flight.depDate).isAfter(new Date())
					) {
	...
	verifyInboundFlights() {
		if (!this.searchForm.oneWay) {
			...
			this.inboundFlights.push({
				date: new Date(earliestInbound),
				flights: [],
			});
			this.inboundFlights.push({
				date: new Date(this.searchForm.inDate),
				flights: [],
			});
			this.inboundFlights.push({
				date: new Date(latestInbound),
				flights: [],
			});
			...
			for (let flight of this.searchResults.inbound) {
				if (
					this.selectedOutbound == null ||
					moment(flight.depDate).isAfter(
						this.selectedOutbound.depDate
					)
				) {
			...
			if (this.selectedInbound != null) {
				if (
					moment(this.selectedInbound.depDate).isBefore(
						this.selectedOutbound.arrDate
					)
				) {
					this.selectedInbound = null;
				}
	...
	increaseOutDate() {
		...
		if (moment(outDate).isAfter(inDate)) {
			inDate.setDate(
				new Date(inDate).getDate() +
					Math.abs(moment(outDate).diff(inDate, 'days'))
			);
		}
		this.searchForm.outDate = new Date(outDate);
	...
	decreaseInDate() {
		let inDate = new Date(this.searchForm.inDate);
		if (
			moment(new Date(inDate).setDate(inDate.getDate()) - 1).isAfter(
				this.selectedOutbound.arrDate
			)
		) {
	...
	completePassengerNumber(
		flight: ISearchResult,
		adult: number,
		child: number,
		infant: number
	) {
